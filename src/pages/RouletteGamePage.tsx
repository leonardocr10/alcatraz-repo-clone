...
import { Swords, SkipForward, Trophy, Clock, Crown, Users, Zap, MapPin, Send, ChevronDown } from "lucide-react";
...
const RouletteGamePage = () => {
  ...
  // Get next upcoming bosses sorted by time
  ...
  // === ROULETTE LOGIC (kept from original) ===
  ...
  const upcomingBosses = getUpcomingBosses();
    ...
    <div className="space-y-4">
      ...
      {winnerAnnouncement && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/90 backdrop-blur-md animate-fade-in">
          <div className="glass-card glow-gold p-10 text-center max-w-sm mx-4 animate-scale-in">
            ...
            <h2 className="font-display text-3xl font-bold text-gold text-shadow-gold mb-3">{winnerAnnouncement.nickname}</h2>
            ...
            <div className="border-t border-border/40 pt-4">
              ...
              <p className="font-display text-xl font-bold text-primary text-shadow-glow">{winnerAnnouncement.itemName}</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="glass-card p-3 text-center">
        <div className="flex items-center justify-center gap-2">
          ...
          <span className="font-display text-xl font-bold tabular-nums text-foreground">{getBrazilTimeStr()}</span>
          <span className="text-xs text-muted-foreground font-body">BRT</span>
        </div>
      </div>
        ...
        <div className="glass-card overflow-hidden">
          <div className="px-5 py-3 border-b border-border/40 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="font-display text-sm font-bold uppercase tracking-wider">Próximos Boss</span>
            </div>
            <button className="text-xs text-primary font-body flex items-center gap-1 hover:text-primary/80">
              <Send className="w-3 h-3" /> Todos
            </button>
          </div>
          <div className="divide-y divide-border/20">
            {upcomingBosses.slice(0, 8).map(({ boss, schedule, minutesUntil }) => (
              <div key={schedule.id}>
                <button
                  onClick={() => setExpandedBoss(expandedBoss === schedule.id ? null : schedule.id)}
                  className="w-full px-5 py-3 flex items-center gap-3 hover:bg-secondary/30 transition-colors"
                >
                  {boss.image_url ? (
                    <img src={boss.image_url} alt={boss.name} className="w-9 h-9 rounded-lg object-cover border border-border/40" />
                  ) : (
                  ...
                  <div className="flex-1 text-left min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-display font-bold text-gold">{boss.name}</span>
                      {boss.map_level && (
                        <span className="text-[11px] text-muted-foreground font-body">({boss.map_level})</span>
                      )}
                    </div>
                    <p className="text-xs text-muted-foreground font-body">
                      Spawn às {schedule.spawn_time.substring(0, 5)}
                    </p>
                  </div>
                ...
                {expandedBoss === schedule.id && (
                  <div className="px-5 py-3 bg-secondary/20 border-t border-border/20">
                    {boss.description && <p className="text-xs text-muted-foreground font-body mb-2">{boss.description}</p>}
                    {boss.map_image_url && (
                      <img src={boss.map_image_url} alt={`Mapa ${boss.name}`} className="w-full rounded-lg border border-border/40 max-h-48 object-contain" />
                    )}
        ...
        <div className="flex-1 space-y-4">
          {!session ? (
            <div className="glass-card p-12 text-center animate-fade-in">
              ...
              <p className="text-muted-foreground font-display text-sm uppercase tracking-wider">Nenhuma sessão ativa</p>
              <p className="text-xs text-muted-foreground/60 mt-2 font-body">Aguarde o admin iniciar</p>
            </div>
          ) : !currentItem ? (
            <div className="glass-card p-12 text-center animate-fade-in">
              <Trophy className="w-12 h-12 text-gold mx-auto mb-4" />
              <p className="text-gold font-display text-sm uppercase tracking-wider">Sessão Encerrada!</p>
            </div>
          ) : (
            <>
                ...
                <div className="p-5">
                  <div className="flex items-center gap-5 mb-5">
                    <img src={currentItem.roulette_items.image_url} alt={currentItem.roulette_items.name}
                      className="w-20 h-20 rounded-2xl border-2 border-border/40 object-contain bg-muted/50 shrink-0"
                      style={{ boxShadow: '0 0 20px hsl(var(--primary) / 0.15)' }}
                    />
                    <div>
                      <h2 className="font-display text-xl font-bold text-foreground">{currentItem.roulette_items.name}</h2>
                      {currentItem.roulette_items.description && (
                        <p className="text-xs text-muted-foreground mt-1 font-body">{currentItem.roulette_items.description}</p>
                      )}
                    </div>
                  </div>
                  ...
                  {currentItem.is_open && timeLeft > 0 ? (
                    <div className="mb-5">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-xs uppercase tracking-wider text-muted-foreground font-body">Tempo restante</span>
                        <div className="flex items-center gap-2">
                          <Clock className={`w-4 h-4 ${isUrgent ? "text-destructive animate-timer-pulse" : "text-primary"}`} />
                          <span className={`font-display text-2xl font-bold tabular-nums ${isUrgent ? "text-destructive animate-timer-pulse" : "text-primary"}`}>{timeLeft}s</span>
                        </div>
                      </div>
                    ...
                    <div className="text-center py-4 mb-4 bg-secondary/50 rounded-xl">
                      <p className="text-destructive font-display text-sm uppercase tracking-wider">⏱ Rodada Encerrada</p>
                      {currentItem.winner_number && <p className="text-gold font-display text-xs mt-1">Vencedor: #{currentItem.winner_number}</p>}
                    </div>
                  )}
              ...
              {plays.length > 0 && (
                <div className="glass-card overflow-hidden animate-slide-up">
                  <div className="px-5 py-3 border-b border-border/40 flex items-center gap-2">
                    ...
                    <span className="text-xs uppercase tracking-wider text-foreground font-display font-semibold">Top Killers</span>
                  ...
                  <div className="divide-y divide-border/30">
                    {plays.map((p, idx) => {
                      ...
                      return (
                        <div key={p.id} className={`px-5 py-3 flex items-center gap-3 transition-colors ${isMe ? "bg-primary/10" : "hover:bg-secondary/50"}`}>
                          <span className={`w-6 text-center font-display text-xs font-bold ${idx === 0 && p.number > 0 ? "text-gold" : "text-muted-foreground"}`}>{idx + 1}</span>
                          ...
                          <span className={`font-display text-sm font-bold ${p.number === 0 ? "text-muted-foreground" : idx === 0 ? "text-gold" : "text-foreground"}`}>
          ...
          <div className="lg:w-80 shrink-0">
            <div className="glass-card overflow-hidden lg:sticky lg:top-20 animate-slide-up">
              <div className="px-5 py-3 border-b border-border/40 flex items-center gap-2">
                ...
                <span className="text-xs uppercase tracking-wider text-foreground font-display font-semibold">Ganhadores</span>
              </div>
                  ...
                  <div key={w.id} className="px-5 py-4 flex items-center gap-4 hover:bg-secondary/30 transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        ...
                        <span className="text-[10px] text-muted-foreground font-body">{new Date(w.created_at).toLocaleTimeString("pt-BR")}</span>
                      </div>
                    </div>
                  </div>
                ))}
